...
// a2_1=q*dt;
 ...
 if(model == 3){/*VG*/ findfactor(0.0, 0.0, h,  kmax, lp1, lm1, alin1, tp1, tm1);}
    ...
    double *ptprice, double *ptdelta)
{
 PnlVect *t, *y, * v1, *gg, *G, *vv1,*tp1,*tm1,*eomcut,
    *alin1,*eom, *reb, *switcharrow,*al1, *bl1,*dividterm;
 int dims[3], tab[3];
 PnlHmat *Price;
 long int N1=0, Nx, kmax, j, L, n, r_index,k_index;
 
 double pp,phiq0,phiq1;
 double xi,xip,xim,anp,anm,sxi,nup1,num1, abp, abm;
 double t1, t2;
 double angle;
...
double flagCall;
 ...
 dividterm=pnl_vect_create(Nmax+1);

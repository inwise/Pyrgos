...
int fastwienerhopf_cir(int model, long int Nr, PnlVect *mu, PnlVect *qu, double om, int am, int upordown,
...
double flagCall;//=1 for call and =-1 for put
 ...
 //                             Memory allocation
 Lambda1=pnl_vect_create(Nr+1);
 vv1= pnl_vect_create(Nr+1);
 vv0= pnl_vect_create(Nr+1);
 //lam1= pnl_mat_create(Nr+1, Nr+1);
...
/////////////////non zero correlation ///////////////////////
      for(j=1;j<Nr-1;j++)
	  {
		  MLET(lam,j,j-1)=MGET(lam,j,j-1)*dt;
		  MLET(lam,j,j+1)=MGET(lam,j,j+1)*dt;
	   }
		  MLET(lam,Nr-1,Nr-2)=MGET(lam,Nr-1,Nr-2)*dt;
		  MLET(lam,0,1)=MGET(lam,0,1)*dt;
...
///////////////////////////////////////////
 ...
 for(k=0;k<Nr;k++)
  {
	  		LET(vv1,k)=(1-GET(Lambda1,k))*GET(vv0,k);

			if(k>0)
		 {  LET(vv1,k)= GET(vv1,k)+MGET(lam,k,k-1)*GET(vv0,k-1);}
			 if(k<Nr-1)
		 {	LET(vv1,k)= GET(vv1,k)+MGET(lam,k,k+1)*GET(vv0,k+1);}
 }// end loop in k

		for(k=0;k<Nr;k++)
		{	LET(vv1,k)=GET(vv1,k)/exp(GET(sg,k)*dt);
			LET(vv0,k)=GET(vv1,k);
			printf("%.8f \n", GET(vv1,k));
		}
printf("%i \n", L);
  ...
  for(k=0;k<Nr;k++)
	{	LET(ptprice,k) = GET(ptprice,k)+LET(coef,n)*GET(vv1,k);}
 ...
 for(k=0;k<Nr;k++)
  {
	  if(upordown==0) // IF DOWN
      {
		for(i=0;i<Nmax;i++)
		{
			LET(vv1,i)=(1-GET(Lambda1,k))*MGET(prices_all,k,i);

...
from numpy import exp, fft, array, pi, zeros, log, sqrt
...
import scripts.factors_mex as mex
...
def evaluate_option_by_wh(T, H_original, K_original, r_premia, V0, kappa, theta, sigma, rho, N, M):
...
#   time discretization
    ...
    for j in range(len(x_space)):
        for k in range(len(V[N])):
            F_n_plus_1[j, k] = array(G(H_original * exp(x_space[j]), K_original))
        ...
        with profiler():
            for k in range(n+1):
                ...
                if V[n, k] >= treshold:
                    ...
                    phi_plus_array = mex.make_phi_minus(M, dx, omega_plus, gamma, sigma_local, q)
                    phi_minus_array = mex.make_phi_plus(M, dx, omega_minus, gamma, sigma_local, q)
...
if __name__ == "__main__":
    evaluate_option_by_wh(T, H_original, K_original, r_premia, V0, kappa, theta, sigma, rho, N, M)
...
def make_phi_plus(M, dx, omega_minus, gamma, sigma, q):
    x_space = ffts.make_fft_spaces(M, dx)[0]
    u_space = ffts.make_fft_spaces(M, dx)[1]

    def integrand_plus(upsilon_array, gamma, sigma):
        return np.array([log(1 + psi_bs(upsilon + 1j * omega_minus, gamma, sigma) / q) / (upsilon + 1j * omega_minus) ** 2 for upsilon in upsilon_array])

    def F_plus_capital():
        indicator = np.where(x_space <= 0, 1, 0)
        trimmed_x_space = indicator * x_space  # чтобы при "сильно положительных" x не росла экспонента
        integral = ffts.make_rad_ifft(integrand_plus(u_space, gamma, sigma), dx)
...
if __name__ == '__main__':
    ...
    phi_m = make_phi_minus(M, dx, omega_plus)
    phi_p = make_phi_plus(M, dx, omega_minus)

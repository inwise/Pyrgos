...
def make_positions(df):
    ...
    forest_directions = df['<FOREST_UP_DOWN>'].values
    positions = []
    ...
    while index <= len(prices) - 1:
        if is_last_position_closed:
            # BUY opening:   <FOREST_UP_DOWN> = 1 и нет открытых позиций
            if forest_directions[index-1] > 0:
                positions.append('BUY')
                last_position_type = 'LONG_POSITION'
                is_last_position_closed = False
            # SHORT opening: <FOREST_UP_DOWN> = -1 и нет открытых позиций
            if forest_directions[index-1] < 0:
